...
pub fn ensure_salt() -> Result<()> {
    ...
    fs::create_dir_all(salt_dir).context("Failed to create /var/lib/kprotect directory")?;
...
pub fn derive_key() -> Result<[u8; 32]> {
    // Read salt
    let salt = fs::read(SALT_PATH).context(format!("Failed to read salt from {}", SALT_PATH))?;
    ...
    let hostname = fs::read_to_string("/etc/hostname").unwrap_or_else(|_| "localhost".to_string());
...
pub fn decrypt_json<T: serde::de::DeserializeOwned>(encrypted: &[u8], key: &[u8; 32]) -> Result<T> {
    ...
    // Parse JSON
    let data = serde_json::from_slice(&plaintext).context("Failed to parse decrypted JSON")?;
...
pub fn save_encrypted<T: serde::Serialize>(data: &T, path: &str, key: &[u8; 32]) -> Result<()> {
    ...
    // Atomic rename
    fs::rename(&temp_path, path).context(format!("Failed to rename to {}", path))?;
...
pub fn load_encrypted<T: serde::de::DeserializeOwned>(path: &str, key: &[u8; 32]) -> Result<T> {
    let encrypted = fs::read(path).context(format!("Failed to read encrypted file: {}", path))?;
