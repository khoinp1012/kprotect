...
use crate::core::auth::{insert_prefix, insert_suffix, parse_pattern, rebuild_auth_caches};
use crate::core::domain::{ChainTrieNode, PathKey, PatternType};
use crate::crypto;
use crate::logger;
...
pub async fn start_daemon() -> Result<()> {
    ...
    crypto::ensure_salt().context("Failed to initialize salt file")?;

    let key = crypto::derive_key().context("Failed to derive encryption key")?;
    ...
    let auth_map: BpfHashMap<_, u64, u8> = BpfHashMap::try_from(
        bpf.take_map("AUTHORIZED_SIGNATURES")
            .context("Map not found")?,
    )?;
    let shared_auth_map = Arc::new(Mutex::new(Some(auth_map)));

    let red_exact_map: BpfHashMap<_, u64, u8> = BpfHashMap::try_from(
        bpf.take_map("RED_EXACT_MAP")
            .context("RED_EXACT_MAP not found")?,
    )?;
    let red_prefix_map: LpmTrie<_, PathKey, u8> = LpmTrie::try_from(
        bpf.take_map("RED_PREFIX_MAP")
            .context("RED_PREFIX_MAP not found")?,
    )?;
    let red_suffix_map: LpmTrie<_, PathKey, u8> = LpmTrie::try_from(
        bpf.take_map("RED_SUFFIX_MAP")
            .context("RED_SUFFIX_MAP not found")?,
    )?;
    let enrichment_prefix_map: LpmTrie<_, PathKey, u8> = LpmTrie::try_from(
        bpf.take_map("ENRICHMENT_PREFIX_MAP")
            .context("ENRICHMENT_PREFIX_MAP not found")?,
    )?;
    let event_map = bpf.take_map("EVENTS").context("Map not found")?;
    let sig_map: BpfHashMap<_, u32, u64> = BpfHashMap::try_from(
        bpf.take_map("PROCESS_SIGNATURES")
            .context("Map not found")?,
    )?;

    let shared_red_exact = Arc::new(Mutex::new(Some(red_exact_map)));
    let shared_red_prefix = Arc::new(Mutex::new(Some(red_prefix_map)));
    let shared_red_suffix = Arc::new(Mutex::new(Some(red_suffix_map)));
    let shared_enrichment_prefix = Arc::new(Mutex::new(Some(enrichment_prefix_map)));
    let shared_sig_map = Arc::new(Mutex::new(Some(sig_map)));
    ...
    let temp_state_obj = AppState {
        ...
        rules: std::sync::RwLock::new(crate::state::RulesState {
    ...
    let state = Arc::new(temp_state_obj);

    info!(
        "🔍 Starting lineage event monitor NOW (ZERO latency - capturing all early boot processes)"
    );
    ...
    tokio::spawn(async move {
        if let Err(e) = crate::ebpf::events::monitor_ebpf_events(
            event_map,
            state_clone,
            auth_map_clone,
            sig_map_clone,
    ...
    if let Err(e) = sd_notify::notify(false, &[sd_notify::NotifyState::Ready]) {
        warn!("⚠️ Failed to signal systemd readiness: {}", e);
    } else {
    ...
    let daemon_config = config::load_config(&key).unwrap_or_else(|_| {
        info!("Creating default daemon config");
        config::DaemonConfig::default()
    });
    config::save_config(&daemon_config, &key)?;
    info!(
        "✅ Daemon config loaded (event retention: {} days, audit retention: {} days)",
    ...
    let mut zones: migration::ZonesFile =
        crypto::load_encrypted("/var/lib/kprotect/configs/zones.enc", &key)
    ...
    let mut enrichment_patterns: Vec<String> =
        crypto::load_encrypted("/var/lib/kprotect/configs/enrichment.enc", &key).unwrap_or_else(
            |_| {
    ...
    info!(
        ...
        zones.red_zones.len(),
        zones.green_zones.len()
    );
    info!("  🔧 Enrichment patterns: {}", enrichment_patterns.len());

    // Load persistent authorized patterns (NEW pattern-based authorization)
    let authorized_patterns: Vec<AuthorizedPattern> =
        crypto::load_encrypted(AUTHORIZED_PATTERNS_PATH, &key).unwrap_or_else(|_| {
    ...
    let notification_rules: Vec<kprotect_common::NotificationRule> =
        crypto::load_encrypted(NOTIFICATION_RULES_PATH, &key).unwrap_or_else(|_| {
    ...
    // Update Notification Manager with loaded rules
    state
        .notification_manager
        .update_rules(notification_rules)
        .await;
    ...
    rebuild_auth_caches(
        &authorized_patterns,
        &mut auth_exact_cache,
        &mut auth_suffix_cache,
    );

    // Update state with loaded configs
    {
        let mut rules = state.rules.write().unwrap();
        rules.authorized_patterns = authorized_patterns;
        rules.sudo_rules = sudo_rules;
        rules.config = daemon_config;
        rules.auth_exact_cache = auth_exact_cache;
        rules.auth_suffix_cache = auth_suffix_cache;

        state
            .event_sequence
            .store(initial_id, std::sync::atomic::Ordering::SeqCst);
    }
    ...
    let state_cleanup_clone = state.clone();
    tokio::spawn(async move {
        let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(3600)); // Hourly
        loop {
            interval.tick().await;
            let retention = {
                let rules = state_cleanup_clone.rules.read().unwrap();
                (
                    rules.config.event_log_retention_days,
                    rules.config.audit_log_retention_days,
                )
            };
            if let Err(e) = logger_clone.cleanup_events(retention.0) {
                warn!("Failed to cleanup event logs: {}", e);
            }
            if let Err(e) = logger_clone.cleanup_audit(retention.1) {
    ...
    tokio::spawn(async move {
        ...
        loop {
            interval.tick().await;
            let cache_size_before = state_clone.lineage_cache.len();
            let removed =
                crate::core::process::cleanup_exited_processes(&state_clone.lineage_cache, false);
            let cache_size_after = state_clone.lineage_cache.len();
            ...
            if removed > 0 {
                info!(
                    "🧹 Lineage cache cleanup: removed {} exited processes ({} -> {})",
            ...
            if hour.is_multiple_of(6) && removed == 0 {
                ...
                if forced_removed > 0 {
                    info!(
                        "🧹 Emergency lineage cleanup: forcefully removed {} exited processes",
                        forced_removed
                    );
                }
            }
...
async fn load_zones(
    state: &Arc<AppState>,
    backup_zones: Option<&migration::ZonesFile>,
) -> Result<()> {
    ...
    info!(
        "Validating {} red zones against limit of {}...",
        zones.red_zones.len(),
        MAX_RED_ZONE_PATTERNS
    );
    ...
    for pattern in &zones.red_zones {
        match parse_pattern(pattern)? {
            PatternType::Prefix(p) => {
                let mut guard = state.red_prefix.lock().await;
            ...
            PatternType::Suffix(s) => {
                let mut guard = state.red_suffix.lock().await;
                if let Some(ref mut m) = *guard {
                    insert_suffix(m, &s)?;
                }
            ...
            PatternType::Exact(e) => {
                let mut guard = state.red_exact.lock().await;
...
async fn load_enrichment_patterns(
    ...
    // 1. Always load Hardcoded System Defaults (Hidden from UI)
    info!("🛡️ Loading system-default enrichment patterns (Quick Sudo requirements)");
    for pattern in SYSTEM_ENRICHMENT_PATTERNS {
        if let Ok(PatternType::Prefix(p)) = parse_pattern(pattern) {
            insert_prefix(map, &p)?;
            info!("  ✓ System Prefix: {}", pattern);
        }
    }

    // 2. Load User/Config patterns
    let patterns = if let Some(backup) = backup_patterns {
        if !backup.is_empty() {
            info!("🔒 Loading user enrichment patterns from encrypted storage");
            backup.clone()
        } else {
            vec![]
        }
    ...
    if patterns.is_empty() {
        info!("⚠️ No user enrichment patterns loaded");
        return Ok(());
    }
    ...
    for pattern in patterns {
        match parse_pattern(&pattern) {
            ...
            Err(e) => {
                error!(
                    "⚠️ Skipping invalid enrichment pattern '{}': {}",
                    pattern, e
                );
                continue;
            }
