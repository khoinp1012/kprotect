...
pub fn is_user_in_group(uid: u32, group_name: &str) -> bool {
    ...
    if user.gid == target_gid { return true; }
...
pub async fn add_zone_to_file(pattern: &str, key: &[u8; 32]) -> Result<String> {
    ...
    if zones.red_zones.contains(&final_pattern.to_string()) { anyhow::bail!("Pattern already exists"); }
...
pub async fn remove_zone_from_file(pattern: &str, key: &[u8; 32]) -> Result<bool> {
    ...
    if zones.red_zones.len() == initial_len { return Ok(false); }
    tokio::task::block_in_place(|| {
        crypto::save_encrypted(&zones, ZONES_ENC, key)
    })?;
    Ok(true)
...
pub async fn add_enrichment_pattern_to_file(pattern: &str, key: &[u8; 32]) -> Result<()> {
    ...
    if patterns.contains(&pattern.to_string()) { anyhow::bail!("Pattern already exists"); }
    if !pattern.ends_with('*') { anyhow::bail!("Enrichment pattern must end with '*' (e.g. /usr/bin/python*)"); }
...
pub async fn remove_enrichment_pattern_from_file(pattern: &str, key: &[u8; 32]) -> Result<bool> {
    ...
    if patterns.len() == initial_len { return Ok(false); }
    tokio::task::block_in_place(|| {
        crypto::save_encrypted(&patterns, ENRICHMENT_ENC, key)
    })?;
    Ok(true)
...
pub fn save_notification_rules(rules: &Vec<kprotect_common::NotificationRule>, key: &[u8; 32]) -> Result<()> {
...
pub fn save_sudo_rules(rules: &Vec<kprotect_common::SudoRule>, key: &[u8; 32]) -> Result<()> {
    crypto::save_encrypted(rules, SUDO_RULES_PATH, key)
        .context("Failed to save sudo rules")
}
